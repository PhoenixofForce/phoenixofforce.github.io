...
<script lang="ts">
  ...
  let filterRef: HTMLSpanElement;
  let textRef: HTMLSpanElement;
  let activeIndex = $state(initialActiveIndex);
  ...
  function handleKey(e: KeyboardEvent, index: number) {
    if (e.key === "Enter" || e.key === " ") {
      ...
      if (liEl) handleClick({ currentTarget: liEl } as unknown as MouseEvent, index);
    }
  }
  ...
  onMount(() => {
    ...
    const ro = new ResizeObserver(() => {
      const cur = navRef?.querySelectorAll("li")[activeIndex] as HTMLElement | undefined;
...
<div class="relative" bind:this={containerRef}>
  <nav class="flex relative" style="transform:translate3d(0,0,0.01px);">
    <ul
      ...
      style="color:white;text-shadow:0 1px 1px hsl(205deg 30% 10% / 0.2);"
    >
      {#each items as item, index (index)}
        <li
          class="rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] text-white {activeIndex ===
          index
            ? 'active'
            : ''}"
        >
          <a
            ...
            class="outline-none py-[0.6em] px-[1em] inline-block">{item.label}</a
          >
        </li>
      {/each}
    </ul>
  </nav>
  <span class="effect filter" bind:this={filterRef}></span>
  <span class="effect text" bind:this={textRef}></span>
</div>
